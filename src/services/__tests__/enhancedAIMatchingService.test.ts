...
jest.mock('../enhancedAIMatchingService', () => {
  const { EnhancedAIMatchingService } = jest.requireActual(
...
describe('EnhancedAIMatchingService', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should initialize with custom options', async () => {
      ...
      expect(analytics?.metadata.totalDocuments).toBe(
        20 * SYNTHETIC_AGENCIES.length
      );
    });
    ...
    it('should not re-initialize if already initialized', async () => {
      ...
      expect(afterAnalytics?.metadata.totalRequests).toBe(
        initialAnalytics?.metadata.totalRequests
      );
    });
    ...
    it('should handle initialization errors gracefully', async () => {
      ...
      service['generator'].generateDataset = jest
        .fn()
        .mockImplementation(() => {
  ...
  describe('Document Matching', () => {
    ...
    it('should find matches for police-related queries', async () => {
      const result = await service.findMatches(
        'test-request-1',
        'police use of force incident reports body camera'
      );
      ...
      const policeRelated = result.candidates.filter(
        c =>
    ...
    it('should find matches for fire department queries', async () => {
      const result = await service.findMatches(
        'test-request-2',
        'fire emergency response times call volumes incidents'
      );
      ...
      const fireRelated = result.candidates.filter(
        c =>
          c.agency.toLowerCase().includes('fire') ||
          c.keyPhrases.some(phrase => phrase.toLowerCase().includes('fire')) ||
          c.keyPhrases.some(phrase =>
            phrase.toLowerCase().includes('emergency')
          )
      );
    ...
    it('should find matches for financial queries', async () => {
      const result = await service.findMatches(
        'test-request-3',
        'budget expenditure contractor payments financial reports'
      );
      ...
      const financeRelated = result.candidates.filter(
        c =>
          c.agency.toLowerCase().includes('finance') ||
          c.keyPhrases.some(phrase =>
            phrase.toLowerCase().includes('budget')
          ) ||
          c.keyPhrases.some(phrase =>
            phrase.toLowerCase().includes('expenditure')
          )
      );
    ...
    it('should filter by agencies when specified', async () => {
      ...
      result.candidates.forEach(candidate => {
        const hasPoliceOrFire =
          candidate.agency.toLowerCase().includes('police') ||
    ...
    it('should return empty results for queries with no matches', async () => {
      const result = await service.findMatches(
        'test-request-7',
        'alien spaceship ufo extraterrestrial',
        {
          minConfidence: 0.5,
        }
      );
  ...
  describe('Match Explanation', () => {
    ...
    it('should provide detailed match explanations', async () => {
      const result = await service.findMatches(
        'test-request-9',
        'police incident reports investigations'
      );
    ...
    it('should extract meaningful search terms', async () => {
      const result = await service.findMatches(
        'test-request-10',
        'police use of force and body camera footage reports'
      );
  ...
  describe('Data Access Methods', () => {
    ...
    it('should return dataset analytics', () => {
      ...
      expect(analytics?.metadata.totalDocuments).toBe(
        12 * SYNTHETIC_AGENCIES.length
      );
      expect(analytics?.metadata.agencies).toEqual(
        SYNTHETIC_AGENCIES.map(a => a.id)
      );
    ...
    it('should find pre-matched documents for synthetic requests', async () => {
      const requests = service.getSyntheticRequests();
      const requestWithMatches = requests.find(
        r => r.expectedMatches && r.expectedMatches.length > 0
      );

      if (requestWithMatches && requestWithMatches.id) {
        const result = await service.findPreMatchedDocuments(
          requestWithMatches.id
        );
  ...
  describe('Data Regeneration', () => {
    it('should regenerate data with new options', async () => {
      ...
      expect(newAnalytics?.metadata.totalDocuments).toBe(
        8 * SYNTHETIC_AGENCIES.length
      );

      // Should have different data
      expect(newAnalytics?.metadata.generatedAt).not.toBe(
        initialAnalytics?.metadata.generatedAt
      );
    });
    ...
    it('should clear existing data during regeneration', async () => {
      ...
      const intersection = new Set(
        [...initialIds].filter(id => newIds.has(id))
      );
  ...
  describe('Error Handling', () => {
    it('should handle searches before initialization', async () => {
      ...
      const result = await uninitializedService.findMatches(
        'auto-init-test',
        'test query'
      );
    ...
    it('should handle empty or invalid queries', async () => {
      ...
      const whitespaceResult = await service.findMatches(
        'whitespace-test',
        '   '
      );
    ...
    it('should handle extreme parameter values gracefully', async () => {
      ...
      const highMaxResult = await service.findMatches(
        'high-max-test',
        'test query',
        {
          maxResults: 1000,
      ...
      expect(highMaxResult.candidates.length).toBeLessThanOrEqual(
        3 * SYNTHETIC_AGENCIES.length
      );

      // Very high minConfidence
      const highConfidenceResult = await service.findMatches(
        'high-conf-test',
        'test query',
        {
          minConfidence: 0.99,
