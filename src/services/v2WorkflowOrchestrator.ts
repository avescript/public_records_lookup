...
export class V2WorkflowOrchestrator {
   ...
   */
  async getWorkflowState(requestId: string): Promise<{
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/workflow/${requestId}/state`,
        {
          method: 'GET',
          headers: {
  ...
  async saveStepProgress(
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/workflow/${requestId}/step/${step}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        throw new Error(`Failed to save step progress: ${response.statusText}`);
      }
    } catch (error) {
   ...
   * Update current step
   */
  async updateCurrentStep(requestId: string, step: number): Promise<void> {
    try {
      const response = await fetch(
        `${this.baseUrl}/workflow/${requestId}/current-step`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update current step: ${response.statusText}`
        );
      }
    } catch (error) {
