...
export class AgencyAnalyticsService {
  ...
  private async generateAgencyMetrics(
    ...
    const [requestMetrics, documentMetrics, costMetrics, performanceKPIs, systemHealth] = 
      await Promise.all([
  ...
  private async generatePerformanceKPIs(agencyId: string): Promise<PerformanceKPIs> {
    return {
  ...
  private generateProcessingTimeDistribution(): ProcessingTimeDistribution[] {
    const ranges = ['0-15 min', '15-30 min', '30-60 min', '60-120 min', '120+ min'];
    const total = 100;
  ...
  private generateRequestTypeBreakdown(): RequestTypeMetric[] {
    const types = ['FOIA Request', 'Police Report', 'Court Record', 'Property Record', 'Other'];
  ...
  private generateStatusDistribution(): StatusMetric[] {
    const statuses = ['Completed', 'In Progress', 'Under Review', 'Rejected', 'On Hold'];
  ...
  private generateFormatDistribution(): FormatMetric[] {
    ...
    return formats.map(({ format, ocrRequired }) => ({
      ...
      averageProcessingTime: this.generateRandomFloat(ocrRequired ? 5 : 1, ocrRequired ? 15 : 5),
      ocrRequired
    }));
  }
  ...
  private generateBenchmarkComparisons(): BenchmarkComparison[] {
    const metrics = ['Response Time', 'Completion Rate', 'User Satisfaction', 'Cost Efficiency'];
  ...
  private generateActiveAlerts(): Alert[] {
    const alerts: Alert[] = [];
    const alertTypes = [
      { severity: 'high' as const, title: 'High Response Time', description: 'Average response time exceeds 150ms' },
      { severity: 'medium' as const, title: 'Storage Usage', description: 'Storage usage above 80% threshold' },
      { severity: 'low' as const, title: 'Pending Reviews', description: '15 documents awaiting manual review' },
    ];
  ...
  private generateRecentIssues(): SystemIssue[] {
    ...
    const issueTypes = [
      { title: 'OCR Service Timeout', impact: 'medium' as const, resolved: true },
      { title: 'Database Connection Slow', impact: 'low' as const, resolved: true },
    ...
    issueTypes.forEach((issue, index) => {
      if (Math.random() > 0.5) {
        issues.push({
          ...
          resolutionTime: issue.resolved ? this.generateRandomCount(15, 240) : undefined
        });
      }
    });
