...
describe('RequestForm', () => {
  it('renders all form fields', () => {
    ...
    expect(
      screen.getByRole('textbox', { name: /request title/i })
    ).toBeInTheDocument();
    const comboboxes = screen.getAllByRole('combobox');
    expect(comboboxes).toHaveLength(2); // Department select and DateRangePicker preset select
    expect(
      screen.getByRole('textbox', { name: /request description/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('textbox', { name: /contact email/i })
    ).toBeInTheDocument();
  ...
  it('displays validation errors for empty required fields on form submission', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/please select a department/i)
      ).toBeInTheDocument();
  ...
  it('allows typing in text fields', async () => {
    ...
    await user.type(
      descriptionInput,
      'This is a test description for the public records request.'
    );
