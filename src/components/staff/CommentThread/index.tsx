...
const NewThreadDialog: React.FC<NewThreadDialogProps> = ({
  ...
  const [threadType, setThreadType] =
    useState<CommentThreadType['threadType']>('general_comment');
  const [content, setContent] = useState('');
  const [priority, setPriority] =
    useState<CommentThreadType['priority']>('medium');
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={e =>
                  setThreadType(
                    e.target.value as CommentThreadType['threadType']
                  )
                }
                label='Thread Type'
              >
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              ...
              <Select
                value={priority}
                onChange={e =>
                  setPriority(e.target.value as CommentThreadType['priority'])
                }
                label='Priority'
              >
...
const ThreadCard: React.FC<ThreadCardProps> = ({
  ...
  return (
    <Card
      sx={{
        mb: 2,
        border: thread.status === 'open' ? '2px solid #ed6c02' : undefined,
      }}
    >
      <CardContent>
        {/* Thread Header */}
...
export const CommentThreadComponent: React.FC<CommentThreadProps> = ({
  ...
  const handleResolveThread = async (threadId: string) => {
    try {
      const updatedThread = await legalReviewService.updateThreadStatus(
        threadId,
        'resolved'
      );
      await loadThreads();
  ...
  return (
    <Box>
      ...
      <Box
        ...
        mb={3}
      >
        <Typography variant='h6'>Comment Threads ({threads.length})</Typography>
        <Button
          variant='contained'
          startIcon={<AddIcon />}
            ...
            <Grid item xs={6} sm={3}>
              <Paper sx={{ p: 2, textAlign: 'center' }}>
                <Typography variant='h4' color='error.main'>
                  {
                    threads.filter(t => t.threadType === 'change_request')
                      .length
                  }
                </Typography>
      ...
      {threads.length === 0 ? (
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant='body2' color='text.secondary' paragraph>
            Start a conversation by creating the first comment thread for this
            record.
          </Typography>
          <Button
            variant='contained'
        ...
        threads.map(thread => (
          <ThreadCard
            key={thread.id}
            thread={thread}
            onAddComment={(content, isResolution) =>
              handleAddComment(thread.id, content, isResolution)
            }
